...
void WriteSpiCommand(volatile unsigned int command) 
{
  ...
  for (Bit = 0; Bit < 8; Bit++)								// 8 Bit Write
  {
	...
	if((command&0x80)>>7) LCD_SDATA_HIGH();  
	else LCD_SDATA_LOW();
...
{
  if (state == 1)
     LCD_BL_HIGH();											// Black Light ON 
  else
     LCD_BL_LOW();											// Black Light OFF 
}
// *****************************************************************************
// InitLcd.c
//
...
// Author: Olimex, James P Lynch August 30, 2007
// *****************************************************************************
void LCDWrite130x130bmp(const unsigned char *ptr) 
{
  long j; // loop counter
